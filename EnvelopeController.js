...
EnvelopeController.prototype.plot = function () {
	...
	for (i = 0; i < this.data.value.length; i += 1) {
...
EnvelopeController.prototype.addPoint = function (x, y) {
	var newPointIndex = 0, newData, coordinates, coordinatesNext;

	while (newPointIndex < this.data.value.length && this.data.value[newPointIndex][0] <= x) {
		newPointIndex += 1;
	}
	newData = this.data.value.slice(0);
	newData.splice(newPointIndex, 0, [x, y]);
	this.data.setValue(newData);

	coordinates = this.getSvgCoordinates(this.data.value[newPointIndex]);
	coordinatesNext = this.getSvgCoordinates(this.data.value[newPointIndex + 1]);
...
EnvelopeController.prototype.deletePoint = function (point) {
	var pointIndex, deleteLine, deletePoint;
	for (pointIndex = 0; pointIndex < this.points.length; pointIndex += 1) {
	...
	deleteLine = this.lines[pointIndex];
	deletePoint = this.points[pointIndex];
...
EnvelopeController.prototype.sizeInPixels = function () {
	...
	unit = this.controller.height.baseVal.SVG_LENGTHTYPE_PX;
	this.controller.height.baseVal.convertToSpecifiedUnits(unit);
	this.controller.width.baseVal.convertToSpecifiedUnits(unit);
	height = this.controller.height.baseVal.valueInSpecifiedUnits;
	width = this.controller.width.baseVal.valueInSpecifiedUnits;
...
EnvelopeController.prototype.mouseDownHandler = function (event) {
    var pixelCoordinates, svgSize, selectedPointIndex;
	if (event.target.nodeName === 'circle') {
		this.pointFocus(event);
		this.selectedPoint = event.target;
	} else if (event.target.nodeName === 'rect' || event.target.nodeName === 'line') {
		pixelCoordinates = this.cursorPosition(event);
		svgSize = this.sizeInPixels();
		selectedPointIndex = this.addPoint(pixelCoordinates.x / svgSize.width, (svgSize.height - pixelCoordinates.y) / svgSize.height);
...
EnvelopeController.prototype.movePoint = function (point, event) {
	var pixelCoordinates, svgSize, pointIndex;

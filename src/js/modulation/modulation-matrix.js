...
class ModulationMatrix {
    ...
    connect (connections, connectionPatch, target) {
        ...
        switch (type) {
            case MODULATOR_TYPE.LFO:
                sourceArray = this.sources.lfos;
                break;

            case MODULATOR_TYPE.MORSE:
                sourceArray = this.sources.morse;
                break;

            case MODULATOR_TYPE.STEPS:
    ...
    patcher (patchable) {
        ...
        for (let key in patchableTargets) {
            ...
            const parameterEnvelopes = parameter.filter(p => {
                return (
                    p.enabled
                    && p.source.type === MODULATOR_TYPE.ENVELOPE
            ...
            const parameterLfos = parameter.filter(p => {
                return (
                    p.enabled
                    && p.source.type === MODULATOR_TYPE.LFO
                    && lfos[p.source.index]
                );
            });
            ...
            const parameterSteps = parameter.filter(p => {
                return (
                    p.enabled
                    && p.source.type === MODULATOR_TYPE.STEPS
            ...
            const parameterMorse = parameter.filter(p => {
                return (
                    p.enabled
                    && p.source.type === MODULATOR_TYPE.MORSE

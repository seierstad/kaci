    ...
    constructor (...args) {
        ...
        for (const moduleName in configuration) {
            ...
            for (const parameterName in targetModule) {
                ...
                let patchPathValue;
                if (patchPath) {
                    patchPathValue = patchPath.reduce((acc, curr) => acc[curr], this.patchState[moduleName]);
                }
                ...
                const value = patchPathValue || patchValue || playStateValue || defaultValue;
                ...
                if (mod !== modNew) {
                    for (const paramName in mod) {
                        ...
                        const {
                            [modName]: {
                                [paramName]: {
                                    patchPath: patchValuePath
                                } = {}
                            } = {}
                        } = this.configuration;
                        if (patchValuePath) {
                            param = patchValuePath.reduce((acc, curr) => acc[curr], mod);
                            newParam = patchValuePath.reduce((acc, curr) => acc[curr], modNew);
                        } else {
                            param = mod[paramName];
                            newParam = modNew[paramName];
                        }

...
class KeyboardInput {
    ...
    keyDownHandler (event) {
        ...
        if (index !== -1) {
            ...
            event.keyCode === this.layout.controls.CHORD_SHIFT_TOGGLE
            && !this.pressedControlKeys[this.layout.controls.CHORD_SHIFT_TOGGLE]
        ) {
            this.store.dispatch({
                "type": CHORD_SHIFT.ENABLE
            });
            event.preventDefault();
            event.stopPropagation();

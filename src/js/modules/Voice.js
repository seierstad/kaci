...
const Voice = function (context, patch, frequency, options, store) {
    const that = this;
    ...
    const createVoiceLfo = function (lfoPatch, index) {
    ...
    for (let i = 0, j = patch.envelopes.length; i < j; i += 1) {
    ...
    const getHandler = function (module, parameter) {
        let result;
        switch (parameter) {
            case "waveform":
                result = function (evt) {
    ...
    const addVoiceEventListeners = function () {
        for (let i = 0, j = eventHandlers.length; i < j; i += 1) {
            context.addEventListener(eventHandlers[i].eventName, eventHandlers[i].handler);
        }
    };
    const getRemoveEventFunction = function () {
